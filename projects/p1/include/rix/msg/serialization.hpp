...
namespace rix
{
    namespace msg
    {
        namespace detail
        {
            ...
            {
                static_assert(std::is_arithmetic<T>::value, "T must be an arithmetic type");
                memcpy(dst + offset, &src, size_number(src));
                offset += size_number(src);
            }
            ...
            inline void serialize_string(uint8_t *dst, size_t &offset, const std::string &src)
            {
                ...
                memcpy(dst + offset, &len, size_number(len));
                memcpy(dst + offset+4, src.data(), src.size());
                offset += size_string(src);
            }
            ...
            {
                // for( const T& num : src) {
                //     memcpy(dst+offset, &num, size_number(num) );
                //     offset += size_number(num);
                // }   
                memcpy(dst + offset, src.data(), size_number_array(src));
                offset += size_number_array(src);
            }
            ...
            {
                static_assert(std::is_base_of<Message, T>::value, "T must derive from Message");
                for (const T &message : src)
                {
                    message.serialize(dst, offset);
                }
            }
                                                ...
                                                const std::vector<T> &src)
            {
                ...
                uint32_t len = static_cast<uint32_t>(src.size());
                memcpy(dst+offset, &len, size_number(len));
                offset += size_number(len);

                for(const T &num : src ) {
                    serialize_number(dst, offset, num);
                }
            }
            ...
            {
                static_assert(std::is_arithmetic<T>::value, "T must be an arithmetic type");
                if (offset + size_number(dst) > size)
                {
                    return false;
                }
                memcpy(&dst, src + offset, size_number(dst));
                offset += size_number(dst);
                return true;
            }
                                           ...
                                           size_t &offset)
            {
                ...
                dst.resize(len);
                memcpy(dst.data(), src + offset, len);
                offset += len;
                return true;
            }
            ...
            inline bool deserialize_message(Message &dst, const uint8_t *src, size_t size,
                                            size_t &offset)
            {
                if (offset + size_message(dst) > size)
                {
                    return false;
                }
                dst.deserialize(src, size, offset);
                return true;
            }
            ...
            {
                static_assert(std::is_arithmetic<T>::value, "T must be an arithmetic type");
                if (offset + size_number_array(dst) > size)
                {
                    return false;
                }
                memcpy(&dst, src + offset, size_number_array(dst));
                offset += size_number_array(dst);
                return true;
            }
            ...
            {
                static_assert(std::is_base_of<Message, T>::value, "T must derive from Message");
                if (offset + size_message_array(dst) > size)
                {
                    return false;
                }
                for (T &message : dst)
                {
                    message.deserialize(src, size, offset);
                }
                return true;
            }
                                                  ...
                                                  size_t size, size_t &offset)
            {
                static_assert(std::is_arithmetic<T>::value, "T must be an arithmetic type");
                uint32_t len;
                if (offset + size_number(len) > size)
                {
                    return false;
                }
                memcpy(&len, src + offset, size_number(len));
                offset += size_number(len);

                if(offset + size_number_vector(dst)>size){
                    return false;
                }
                dst.resize(len);
                memcpy(dst.data(), src + offset, size_number_vector(dst));
                offset += size_number_vector(dst);
                return true;
            }
                                                  ...
                                                  size_t size, size_t &offset)
            {
                ...
                if(offset + size_string_vector(dst)>size){
                    return false;
                }
                dst.resize(len);
                for( string &s : dst) {
                                                   ...
                                                   size_t size, size_t &offset)
            {
                ...
                if(offset + size_message_vector(dst)>size){
                    return false;
                }
                dst.resize(len);
                for(T &message: dst) {
                ...
                offset += size_message_vector(dst);
                return true;
            }
        } // namespace detail
    } // namespace msg
